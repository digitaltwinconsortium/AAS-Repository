...
namespace MS.Internal.IO.Packaging.AdminShell
{
    ...
    internal class XmlDigitalSignatureProcessor
    {
        ...
        internal XmlDigitalSignatureProcessor(PackageDigitalSignatureManager manager,
            System.IO.Packaging.PackagePart signaturePart, PackageDigitalSignature packageSignature) : this(manager, signaturePart)
        {
        ...
        internal static PackageDigitalSignature Sign(
            ...
            System.IO.Packaging.PackagePart signaturePart,
                ...
                try
                {
                    ...
                    foreach (PartManifestEntry partEntry in _partEntryManifest)
                    {
                        ...
                        else    // Part entry - inspect raw stream
                        {
                            ...
                            if (!PackageDigitalSignatureManager.AreTypeAndSubTypeEqual(partEntry.ContentType, new ContentType(part.ContentType)))
                            {
        ...
        /// </summary>
        internal System.IO.Packaging.PackagePart SignaturePart
        {
            get
            {
        ...
        internal static Stream GenerateRelationshipNodeStream(IEnumerable<PackageRelationship> relationships)
        {
                ...
                System.Text.Encoding.UTF8))
            {
                ...
                WriteRelationshipsAsXml(writer, relationships,
        ...
        internal static void WriteRelationshipsAsXml(XmlWriter writer, IEnumerable<PackageRelationship> relationships, bool alwaysWriteTargetModeAttribute, bool inStreamingProduction)
        {
            foreach (PackageRelationship relationship in relationships)
            {
                ...
                writer.WriteAttributeString("Target", relationship.TargetUri.OriginalString);
                ...
                if (alwaysWriteTargetModeAttribute || relationship.TargetMode == TargetMode.External)
                    writer.WriteAttributeString("TargetMode", relationship.TargetMode.ToString());
            ...
            System.IO.Packaging.PackagePart signaturePart)
        {
            ...
            Debug.Assert(signaturePart != null);
        ...
        private KeyInfo GenerateKeyInfo(AsymmetricAlgorithm key, X509Certificate2 signer)
        {
            ...
            else
            {
                if (key is DSA)
                    ...
                    throw new ArgumentException("CertificateKeyTypeNotSupported");
            }
        ...
        private static AsymmetricAlgorithm GetPrivateKeyForSigning(X509Certificate2 signer)
        {
            ...
            Debug.Assert(!signer.HasPrivateKey);
        ...
        //
        //------------------------------------------------------
        private System.IO.Packaging.PackagePart _signaturePart;

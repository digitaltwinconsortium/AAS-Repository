...
namespace MS.Internal.IO.Packaging.AdminShell
{
    ...
    internal static class XmlSignatureManifest
    {
            ...
            out List<PackageRelationshipSelector> relationshipManifest)
        {
            ...
            while (reader.Read() && (reader.MoveToContent() == XmlNodeType.Element))
            {
                    ...
                    && reader.Depth == 2)
                {
                    ...
                    throw new XmlException("UnexpectedXmlTag");
            }
        ...
        private static Uri ParsePartUri(XmlReader reader, out ContentType contentType)
        {
            ...
            if (partUri == null)
                throw new XmlException("RequiredXmlAttributeMissing");
        ...
        private static Uri ParsePartUriAttribute(String attrValue, out ContentType contentType)
        {
            ...
            contentType = new ContentType("application/octet-stream");             // out argument must always be set
            ...
            if (index > 0)
            {
                try
                {
                    ...
                    uri = System.IO.Packaging.AdminShell.PackUriHelper.ValidatePartUri(new Uri(attrValue.Substring(0, index), UriKind.Relative));
                }
            ...
            IEnumerable<PackageRelationshipSelector> relationshipSelectors)
        {
            ...
            Uri containerRelationshipPartName = System.IO.Packaging.PackUriHelper.CreatePartUri(new Uri("/_rels/.rels", UriKind.Relative));
            if (System.IO.Packaging.PackUriHelper.ComparePartUri(relationshipPartName, containerRelationshipPartName) == 0)
                relationshipPartString = containerRelationshipPartName.ToString();
            else
                relationshipPartString = System.IO.Packaging.AdminShell.PackUriHelper.GetStringForPartUri(relationshipPartName);
            ...
            foreach (PackageRelationshipSelector relationshipSelector in relationshipSelectors)
            {
                switch (relationshipSelector.SelectorType)
                {
                    ...
                    case PackageRelationshipSelectorType.Type:
                        ...
                        break;
                    default:
                        Debug.Assert(false, "This option should never be executed");
                        break;
                }
            }
            ...
            Uri partName)
        {
            ...
            XmlAttribute uriAttr = xDoc.CreateAttribute(XTable.Get(XTable.ID.UriAttrName));
            uriAttr.Value = System.IO.Packaging.AdminShell.PackUriHelper.GetStringForPartUri(partName) + _contentTypeQueryStringPrefix + part.ContentType;
            reference.Attributes.Append(uriAttr);

...
using MS.Internal.IO.Packaging.AdminShell;
...
namespace System.IO.Packaging.AdminShell
{
    /// <summary>
        ...
        public String HashAlgorithm
        {
            ...
            set
            {
                ...
                if (value == String.Empty)
                    throw new ArgumentException("UnsupportedHashAlgorithm");
...
#pragma warning disable 6506
            ...
            System.IO.Packaging.PackagePart newSignaturePart = null;
            try
            {
                // create the new part
            ...
            if (_certificateEmbeddingOption == CertificateEmbeddingOption.InCertificatePart)
            {
                ...
                Uri certificatePartName = System.IO.Packaging.PackUriHelper.CreatePartUri(new Uri(
...
#pragma warning restore 6506
        ...
        public PackageDigitalSignature Countersign(X509Certificate certificate, IEnumerable<Uri> signatures)
        {
            ...
            {
                System.IO.Packaging.PackagePart part = _container.GetPart(uri);
                if (!AreTypeAndSubTypeEqual(new ContentType(part.ContentType), XmlDigitalSignatureProcessor.ContentType))
                    throw new ArgumentException("CanOnlyCounterSignSignatureParts");
            }
            ...
            try
            {
                ...
                for (int i = 0; i < _signatures.Count; i++)
                {
                    ...
                    foreach (PackageRelationship r in p.GetRelationshipsByType(CertificatePart.RelationshipType))
                    {
                        ...
                        _container.DeletePart(System.IO.Packaging.PackUriHelper.ResolvePartUri(r.SourceUri, r.TargetUri));                 // will not throw if part not found
                    }
            ...
            if (parts != null)
            {
                foreach (Uri partUri in parts)
                {
                    if (!_container.PartExists(partUri))
                    {
                        ...
                        throw new ArgumentException("PartToSignMissing");
                    }
                }
            }
            ...
            IEnumerable<System.Security.Cryptography.Xml.Reference> objectReferences)
        {
            ...
            if (signatureObjects != null)
            {
                ...
                foreach (DataObject obj in signatureObjects)
                {
                    ...
                    if (String.CompareOrdinal(obj.Id, XTable.Get(XTable.ID.OpcAttrValue)) == 0)
                        throw new ArgumentException("SignaturePackageObjectTagMustBeUnique");

                    // check for duplicates
                    //if (ids.Contains(obj.Id))
                    if (ids.Exists(new StringMatchPredicate(obj.Id).Match))
                        throw new ArgumentException("SignatureObjectIdMustBeUnique");
                    else
                        ids.Add(obj.Id);
                }
            }
            ...
            if ((signatureId != null) && (signatureId != String.Empty))
            {
                ...
                {
                    throw new ArgumentException("NotAValidXmlIdString " + signatureId + " " + xmlException);
                }
            }
        }
        ...
        private bool DeleteCertificateIfReferenceCountBecomesZeroVisitor(PackageRelationship r, Object context)
        {
            ...
            Uri certificatePartName = System.IO.Packaging.PackUriHelper.ResolvePartUri(r.SourceUri, r.TargetUri);
        ...
        private bool DeleteRelationshipOfTypePackageToOriginVisitor(PackageRelationship r, Object context)
        {
            ...
            Uri targetUri = System.IO.Packaging.PackUriHelper.ResolvePartUri(r.SourceUri, r.TargetUri);
            if (System.IO.Packaging.PackUriHelper.ComparePartUri(targetUri, _originPartName) == 0)
        ...
        private bool DeleteRelationshipToSignature(PackageRelationship r, Object signatureUri)
        {
            ...
            if (System.IO.Packaging.PackUriHelper.ComparePartUri(System.IO.Packaging.PackUriHelper.ResolvePartUri(r.SourceUri, r.TargetUri), uri) == 0)
            {
        ...
        private int GetSignatureIndex(Uri uri)
        {
            ...
            for (int i = 0; i < _signatures.Count; i++)
            {
                if (System.IO.Packaging.PackUriHelper.ComparePartUri(uri, _signatures[i].SignaturePart.Uri) == 0)
                    return i;
            }
            return -1;      // not found
        }
        ...
        private int CertificatePartReferenceCount(Uri certificatePartUri)
        {
            ...
            for (int i = 0; i < _signatures.Count; i++)
            {
                ...
                {
                    // same Uri?
                    if (System.IO.Packaging.PackUriHelper.ComparePartUri(certificatePartUri, _signatures[i].GetCertificatePart().Uri) == 0)
                        ++count;
                }
            }
        ...
        {
            return System.IO.Packaging.PackUriHelper.CreatePartUri(new Uri(_defaultSignaturePartNamePrefix +
            ...
            if (_signatures == null)
            {
                ...
                if (OriginPartExists())
                {
                    ...
                    foreach (PackageRelationship r in relationships)
                    {
                        ...
                        Uri signaturePartName = System.IO.Packaging.PackUriHelper.ResolvePartUri(_originPart.Uri, r.TargetUri);

                        // throw if part does not exist
                        if (!_container.PartExists(signaturePartName))
                            throw new FileFormatException("PackageSignatureCorruption");

                        System.IO.Packaging.PackagePart signaturePart = _container.GetPart(signaturePartName);

                        // ignore future signature types that we do not recognize
                        if (AreTypeAndSubTypeEqual(new ContentType(signaturePart.ContentType), XmlDigitalSignatureProcessor.ContentType))
                        {
                            // parse it
            ...
            if (!_originSearchConducted)
            {
                try
                {
                    ...
                    foreach (PackageRelationship r in containerRelationships)
                    {
                        ...
                        if (AreTypeAndSubTypeEqual(new ContentType(p.ContentType), _originPartContentType))
                        {
        ...
        /// <returns></returns>
        public static bool AreTypeAndSubTypeEqual(ContentType contentType1, ContentType contentType2, bool allowParameterValuePairs = false)
        {
            bool result = false;
            ...
            if (contentType2 != null)
            {
                ...
                {
                    //Return false if content type 1 has parameters
                    StringDictionary contentTypeEnumerator1 = contentType1.Parameters;
                    if (contentTypeEnumerator1.Count > 0)
                        return false;

                    //Return false if content type 2 has parameters
                    StringDictionary contentTypeEnumerator2 = contentType2.Parameters;
                    if (contentTypeEnumerator2.Count > 0)
                        return false;
                }
                ...
                result = (String.Compare(contentType1.MediaType, contentType2.MediaType, StringComparison.OrdinalIgnoreCase) == 0 &&
                          String.Compare(contentType1.CharSet, contentType2.CharSet, StringComparison.OrdinalIgnoreCase) == 0);
            }
            return result;
        }
        ...
        private static Uri _defaultOriginPartName = System.IO.Packaging.PackUriHelper.CreatePartUri(new Uri("/package/services/digital-signature/origin.psdsor", UriKind.Relative));
